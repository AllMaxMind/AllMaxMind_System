...
const FullBlueprintContent: React.FC<{ blueprint: Blueprint }> = ({ blueprint }) => (
  <div className="space-y-8 animate-slide-up">
    ...
    {/* P9: Tech Stack hidden from display (still generated by backend for future API use) */}
    {/*
    <div className="p-6 bg-ds-surface rounded-xl border border-ds-border">
      <h3 className="text-lg font-bold mb-3 flex items-center gap-2 text-ds-primary-400">
...
const Phase4: React.FC<Phase4Props> = ({
  ...
  const [isGeneratingBlueprint, setIsGeneratingBlueprint] = useState(true);
  ...
  const generateBlueprintContent = async () => {
    ...
    try {
      ...
      // Track success
      analytics.trackEvent('blueprint_generated', {
      ...
      const errorMessage = error.message?.includes('API Key')
        ? 'Erro de configuração do sistema. Por favor, tente novamente mais tarde.'
        : error.message?.includes('Empty response')
        ? 'Não foi possível gerar o blueprint. Nossa IA está com alta demanda. Tente novamente.'
        : 'Ocorreu um erro ao gerar seu blueprint. Por favor, tente novamente.';

      setBlueprintError(errorMessage);
      setGeneratedBlueprint(null);

      // Track error
      analytics.trackEvent('blueprint_generation_error', {
        problem_id: problemId,
        error: error.message,
        retry_count: retryCount
      });
  ...
  const handleRetryBlueprint = async () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      await downloadBlueprintPDF(generatedBlueprint, {
        problemStatement: problemText,
        companyName: leadForm.company || 'All Max Mind',
      });
      toast.success('Blueprint baixado com sucesso!');

      // Track event
      analytics.trackEvent('blueprint_pdf_downloaded', {
        problem_id: problemId,
        complexity,
  ...
  const handleWhatsAppRedirect = () => {
    // Deep link para WhatsApp Business - usando variável de ambiente
    const phoneNumber = (import.meta as any).env?.VITE_WHATSAPP_PHONE || '5511999999999';
    const complexityLabel = t(`complexity.${complexity}`, { defaultValue: complexity });
    const message = t('whatsapp.message', { complexity: complexityLabel, problemId });
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 animate-fade-in">
      ...
      {/* Loading State */}
      {isGeneratingBlueprint && !generatedBlueprint && (
        <div className="max-w-2xl mx-auto mb-10">
          <div className="glass-card p-8 text-center border border-ds-primary-500/30">
            <LoadingSpinner size="lg" className="mx-auto mb-4" />
            <h3 className="text-xl font-bold text-white mb-2">Gerando seu Blueprint...</h3>
            <p className="text-ds-text-secondary">
              Nossa IA está analisando seu problema e criando uma solução personalizada.
      ...
      {/* Error State */}
      {blueprintError && !isGeneratingBlueprint && (
        <div className="max-w-2xl mx-auto mb-10">
          <div className="glass-card p-8 border border-red-500/30 bg-red-900/10">
            <div className="flex flex-col items-center text-center">
              ...
              {retryCount > 0 && (
                <p className="text-xs text-ds-text-tertiary mt-4">
                  Tentativa {retryCount} de 3. Se o problema persistir, entre em contato conosco.
                </p>
              )}
            </div>
          </div>
      ...
      {/* Main Content - Only show when blueprint is ready or loading */}
      {(generatedBlueprint || isGeneratingBlueprint) && !blueprintError && (
      ...
      {!isAuthenticated && !showFullBlueprint && !blueprintError && (
